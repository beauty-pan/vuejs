	...
	methods: {
		...
		subInsurance(event) {
			if($("#confirmQuote").hasClass("disabled")) {
				...
				$("#selPlanWin").hide();
				console.log(sessionStorage.schemeCode);
				console.log(sessionStorage.planCode);
				console.log(sessionStorage.ageCode);
				console.log(sessionStorage.premium);
				checkLogin();
				$("body").removeClass("productPlanWinConts");
				localStorage.setItem("chooseAge",$("#ansered ul:eq(2) .on").text());
				this.$router.push({path: '/Insurance', query: {schemeCode:$("#ansered ul:eq(0) .on").next().val(),planCode:$("#ansered ul:eq(0) .on").next().val(),ageCode:$("#ansered ul:eq(0) .on").next().val(),premium:sessionStorage.premium}});
		...
		getQuickQuote: function(data, e) {
			...
			doPost(requestUrl, JSON.stringify(quoteData)).done(function(res) { //接口返回数据
				...
				vm.premium = premium;
				$("#quote").text(premium);
				sessionStorage.schemeCode = quoteData.schemeCode;
				sessionStorage.planCode = quoteData.planCode;
				sessionStorage.ageCode = quoteData.ageCode;
				sessionStorage.premium = premium;
				$("#confirmQuote").removeClass("disabled");
			}).fail(function(error) {
