...
var ageCode=sessionStorage.ageCode, planCode=sessionStorage.planCode, schemeCode=sessionStorage.schemeCode;
console.log(ageCode);
    ...
    created: function(){
       ...
       var  requestUrl=BAF.contextPath+"/sgw/v1/products/queryProductProperties";
        ...
        doGet(requestUrl,{"productCode": "AYB0021"}).done(function(res){  //接口返回数据
            ...
            $("#premium").text(sessionStorage.premium); //从产品详情页获取保费
            ...
            var ast=setInterval(function(){
                if(!ageCode || !planCode || !schemeCode){
                    ...
                    doPost(getQuickQuoteurl, data, function () {
                        ...
                        if (msg.premium != sessionStorage.premium && type != 'pressBtn') { //判断当前被保人信息与产品页选择方案是否一致
...
function createQuote() {
    console.log(sessionStorage.ageCode+"-"+sessionStorage.planCode+"-"+sessionStorage.schemeCode);
    /**投保数据**/
    var order=sessionStorage.MyHiOrder;
    var _orderInfo=JSON.parse(order);
    if(_orderInfo.HI.ageCode=="" || _orderInfo.HI.ageCode==null){
        _orderInfo.HI.ageCode=sessionStorage.ageCode;
        _orderInfo.HI.planCode=sessionStorage.planCode;
        _orderInfo.HI.schemeCode=sessionStorage.schemeCode;
    }
